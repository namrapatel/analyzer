/**
 * The MarketCapAnalysis class takes in a UserSelection object and using that, calculates the volume for the 
...
private UserSelection theSelection;
	...
	public VolumeAnalysis(UserSelection selection) {								//Constructor
	...
	public Result perform() {														//Begin perform function
		...
		ArrayList<Object> volume = new ArrayList<Object>();							//Initialize ArrayList to store data from JSON file
		Object[] result = null;
		int count;
		volume.add(coin);
		...
		while (workingDate.before(currentDate)||workingDate.equals(currentDate)) {						//While the date is before or equal to the current date
			volume.add(apiCall.getVolumeForCoin(coin.toLowerCase(), dateFormatter(workingDate)));		//Store the Market Cap value for that coin that day
		...
		if(interval == "Daily") {													//If requested interval is Daily			
			result = volume.toArray();												//Store ArrayList in the result array
		}
			...
			if(size >= 7) {
				...
				for(int i = 1; i < volume.size() -1;i++) {							//For each cell in the market cap ArrayList
					intervalResult += (double)volume.get(i);						//Increase the intervalResult counter
					count ++;
					...
					if(place == result.length -1) {
						if(size < 7) {												//if the interval is less than a week
							for(int j = i; j < volume.size() -1; j++) {				
								intervalResult +=(double)volume.get(j);				//add up the remaining days data
							}
							...
							for(int j = i; j < volume.size() -1; j++) {
								intervalResult +=(double)volume.get(j);				//add up the remaining days
							}
				...
				for(int i = size; i>0; i--) {
					double j = (double) volume.get(count1);
			...
			if(size >= 30) {
				...
				for(int i = 1; i < volume.size() -1;i++) {
					intervalResult += (double)volume.get(i);
					count ++;
					if(count == 30) {
					...
					if(place == result.length -1) {
						if(size < 30) {
							for(int j = i; j < volume.size() -1; j++) {
								intervalResult +=(double)volume.get(j);
							}
